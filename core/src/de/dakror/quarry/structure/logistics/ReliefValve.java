 ...
 */
public class ReliefValve extends FluidTubeStructure implements IRotatable {
    public static final FluidTubeSchema classSchema = new FluidTubeSchema(0, StructureType.ReliefValve, 10000, 1, 1,
            "reliefvalve", new Items(ItemType.SteelTube, 6, ItemType.SteelIngot, 12), null)
                    .sciences(ScienceType.PlasticMolding)
                    .removeFlags(Flags.Draggable);

    static final TextureRegion top = Quarry.Q.atlas.findRegion("structure_reliefvalve_top");
    ...
    /**
     * Relief Valve only accepts fluid when the output pipe is full.
     * This ensures fluid only flows when there's backup pressure.
     */
    @Override
    public boolean canAccept(ItemType item, int x, int y, Direction dir) {
        // Must be a fluid and not molten metal
        if (!item.categories.contains(ItemCategory.Fluid)
                || Item.base(item) == ItemType._MoltenMetal) {
            return false;
        }

        // Must come from an allowed direction
        if (dir == flowDir.inv() || dir == this.dir) {
            return false;
        }

        // Check if output pipe is full before accepting fluid
        // The output direction is flowDir
        Structure<?> outputStructure = structures[flowDir.ordinal()];
        if (outputStructure != null && outputStructure instanceof FluidTubeStructure) {
            FluidTubeStructure outputTube = (FluidTubeStructure) outputStructure;
            // Only accept fluid if the output pipe is at full capacity
            // We check this by seeing if the output can accept more fluid (if it returns false, it's full)
            // From the output tube's perspective, fluid comes from the inverse direction
            return !outputTube.canAccept(item, x, y, flowDir.inv());
        }

        // If no output structure exists yet, don't accept fluid
        return false;
    }
    ...
    public void draw(SpriteRenderer spriter) {
        // Draw base texture (already orange-tinted in asset)
        ...
        // Draw top layer (already orange-tinted in asset)
